...
class AnnotationMappingRule {
	...
	def private void mapAnnotation(List<GenAnnotationMirror> annotations, Map<String, AnnotationMappingRule> mappingsWithId) {
		...
		if (am == null) {
			if (mode == REMOVE) {
				...
				am = asAnnotationValue(annotations)
			} else {
	 ...
	 */
	def GenAnnotationMirror asAnnotationValue(List<GenAnnotationMirror> annotations) {
		try {
			...
			messageCollector.reportError(
				'''Could not set or add «targetAnnotation» as annotation value «asAvAnnotationValueName» of «asAvAnnotation».''', e, if(currentSrc instanceof Element) currentSrcElement, null, null)
			null
		}
	}
