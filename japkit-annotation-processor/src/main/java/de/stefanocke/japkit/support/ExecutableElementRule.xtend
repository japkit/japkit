...
abstract class ExecutableElementRule extends MemberRuleSupport<ExecutableElement> {
	...
	def protected void setParametersFromMetaAnnotation(GenExecutableElement executableElement, AnnotationMirror triggerAnnotation,
		...
		paramRules.forEach[
			...
			val paramTypeFromAnno = if(paramAnnotation==null) null else resolveType(triggerAnnotation, annotatedClass, generatedClass, paramAnnotation, "type", "typeArgs",
				ruleSrcElement)
			...
			mapAnnotations(ruleSrcElement, paramAnnotationMappings).forEach[param.addAnnotationMirror(it)]
